...
and Unity WebGL are not supported.
...
| `cf-create --prompt <text> [--interpreter] [--turbo] [--seed] [--idempotency-key] [--spawn] [--position] [--reference-controller] [--speed] [--timeout] [--server]` | Generates, imports, and optionally spawns one character. |
...
Version 0.1.0. File issues at the
